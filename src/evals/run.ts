...
import { ValidationError } from "serialization";
...
function fileOrFileInsideHLUtility(file: File): File {
    if (callableIsHumanloopUtility(file)) {
        // @ts-ignore
        const innerFile: File = file.callable!.file! as File;
...
function getFileCallable(file: File, type_: FileType): Function | undefined {
...
export async function runEval(
    ...
    let localEvaluators: LocalEvaluator[];
    try {
    ...
    async function processDatapoint(
        ...
        contextApi.context.with(
            ...
            async () => {
                ...
                try {
                    const output = await callFunction(function_!, type_, datapoint);
...
async function callFunction(
    ...
    let output;
    if ("messages" in datapointDict && datapointDict["messages"] !== null) {
        output = func({
            ...datapointDict["inputs"],
            messages: datapointDict["messages"],
        });
    } else {
        output = func(datapointDict["inputs"]);
    }
...
async function upsertLocalEvaluators({
    ...
    const localEvaluators: LocalEvaluator[] = [];
    if (evaluators) {
