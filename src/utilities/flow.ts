...
import { getTraceId, setDecoratorContext, setTraceId } from "../eval_utils";
import {
            ...
            async () => {
                return opentelemetryTracer.startActiveSpan(
                    ...
                    async (span) => {
                        ...
                        // @ts-ignore Using the original log method not the overloaded one
                        // Practically doing sudo on the client
                        const flowLogResponse = await client.flows._log({
                            path,
                            flow: {
                                attributes,
                            },
